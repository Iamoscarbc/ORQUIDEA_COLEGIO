...
$data=$obj->consultar("
    SELECT
        ...
        vp.numero_voucher AS ,
        vp.imagen_voucher AS imagevoucher,
        vp.fecha AS fecha
    FROM voucherpago vp
    INNER JOIN apoderado ap
    ON ap.idapo=vp.idapoderado
    WHERE idvoucher='".$obj->real_escape_string($id)."'");

foreach($data as $row){
$apoderado=$row['apoderado'];
$numerovoucher=$row['numvocher'];
$imagenvoucher=$row['imagevoucher'];
$fechavoucher=$row['fecha'];
}
?>
<head>
